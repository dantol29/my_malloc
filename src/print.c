...
void show_alloc_mem()
{
    ...
    if (heap.large_zone)
    {
        safe_write(1, "LARGE: 0x", 10);
        ft_put_ptr((uintptr_t)heap.large_zone);
        safe_write(1, "\n", 2);
        ...
        while (current_zone)
        {
            ...
            safe_write(1, "0x", 3);
            ft_put_ptr((uintptr_t)((char *)current_zone + METADATA_SIZE));
            safe_write(1, " - 0x", 6);
            ft_put_ptr((uintptr_t)((char *)current_zone + size));
            safe_write(1, " : ", 4);
            write_size_t(size);
            safe_write(1, " bytes\n", 8);
    ...
    safe_write(1, "TOTAL: ", 8);
    write_size_t(total_bytes);
    safe_write(1, "\n", 2);
}
